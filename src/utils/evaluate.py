...
def build_validation_labels(validation_df: pd.DataFrame) -> pd.DataFrame:
    """
    검증 데이터 셋 기준 label 데이터를 생성하는 함수
    Returns
    -------
    label_df : pd.DataFrame
        컬럼: ['impression_id','clicked_label']
    """
    ...
    SELECT impression_id, LIST(clicked ORDER BY position) AS clicked_label
    FROM table
    GROUP BY 1
    ORDER BY 1
    """).df()
